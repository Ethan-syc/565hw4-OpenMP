...
long *histogram(char *fn_input) {
  ...
  for (m = 0; m < 100; m++) {
    // #pragma omp parallel for collapse(2) num_threads(2) default(shared)
    // private(i,j) reduction(+:histo[:256])
    long histo_private[256] = {0};
#pragma omp parallel firstprivate(histo_private) shared(histo) num_threads(8)
    {
#pragma omp for private(i, j)
      for (i = 0; i < image->row; i++) {
        for (j = 0; j < image->col; j++) {
          // histo[image->content[i][j]]++;
          histo_private[image->content[i][j]]++;
        }
      }
#pragma omp critical
      {
        for (int i = 0; i < 256; i++) {
          histo[i] += histo_private[i];
        }
      }
    }
  }
