...
int hypre_CSRMatrixMatvec(double alpha, hypre_CSRMatrix *A, hypre_Vector *x,
  ...
  Timer_Stop("alpha == 0.0");
  /*-----------------------------------------------------------------------
   * y = (beta/alpha)*y
   *-----------------------------------------------------------------------*/

  temp = beta / alpha;
  Timer_Start("temp != 1.0");
  if (temp != 1.0) {
    if (temp == 0.0) {
      Timer_Start("temp == 0.0");
      for (i = 0; i < num_rows * num_vectors; i++)
        y_data[i] = 0.0;
      Timer_Stop("temp == 0.0");
    } else {
      Timer_Start("temp != 1.0 && temp != 0.0");
      for (i = 0; i < num_rows * num_vectors; i++)
        y_data[i] *= temp;
      Timer_Stop("temp != 1.0 && temp != 0.0");
    }
  }
  Timer_Stop("temp != 1.0");
  /*-----------------------------------------------------------------
  ...
  if (num_rownnz < xpar * (num_rows)) {
    Timer_Start("num_rownnz < xpar * (num_rows)");
    ...
    Timer_Stop("num_rownnz < xpar * (num_rows)");
  } else {
    Timer_Start("num_rownnz >= xpar * (num_rows)");
   ...
   * y = alpha*y
   *-----------------------------------------------------------------*/
  Timer_Start("alpha != 1.0");
  if (alpha != 1.0) {
    for (i = 0; i < num_rows * num_vectors; i++)
      y_data[i] *= alpha;
  }
  Timer_Stop("alpha != 1.0");
...
int hypre_CSRMatrixMatvecT(double alpha, hypre_CSRMatrix *A, hypre_Vector *x,
                           double beta, hypre_Vector *y) {
  double *A_data = hypre_CSRMatrixData(A);
  int *A_i = hypre_CSRMatrixI(A);
  int *A_j = hypre_CSRMatrixJ(A);
...
int hypre_CSRMatrixMatvec_FF(double alpha, hypre_CSRMatrix *A, hypre_Vector *x,
                             double beta, hypre_Vector *y, int *CF_marker_x,
                             int *CF_marker_y, int fpt) {
  double *A_data = hypre_CSRMatrixData(A);
  int *A_i = hypre_CSRMatrixI(A);
  int *A_j = hypre_CSRMatrixJ(A);
  int num_rows = hypre_CSRMatrixNumRows(A);
